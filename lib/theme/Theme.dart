...
String getCapitilizedFirstLetterString(String str) {
  return str == ''
      ? ''
...
getImage(context, bufferfile) {
  if (bufferfile != null) {
    var bufferData = bufferfile;
    var list = List<int>.from(bufferData);
    var temp = Base64Encoder().convert(list);
    var _bytesImage = Base64Decoder().convert(temp);
...
getNetworkImage(context, path) {
  if (path != null && path != null) {
    return Image.network(path,height: 200, fit: BoxFit.fill);
  } else {
    return Container(
